...
func (c *CommentsPG) GetAnimeComments(ctx context.Context, animeID int, page int) ([]*domain.AnimeComment, error) {
	if page*domain.DefaultCommentsPerPage < 0 {
	...
	rows, err := c.pool.Query(ctx, query, animeID, domain.DefaultCommentsPerPage, (page-1)*domain.DefaultCommentsPerPage)
	if err != nil {
...
func (c *CommentsPG) GetNewsComments(ctx context.Context, newsID int, page int) ([]*domain.NewsComment, error) {
	...
	rows, err := c.pool.Query(ctx, query, domain.DefaultCommentsPerPage, (page-1)*domain.DefaultCommentsPerPage)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
