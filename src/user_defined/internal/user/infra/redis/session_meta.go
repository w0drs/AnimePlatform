...
func (r *tokenRepository) SaveSessionMeta(ctx context.Context, jti uuid.UUID, meta domain.SessionMeta, ttl time.Duration) error {
	...
	if err != nil {
		r.logger.Error("error marshalling session meta", "error", err.Error())
	...
	if err != nil {
		r.logger.Error("error setting session meta", "error", err.Error(), "jwtID", jti.String())
		return coreHttp.NewErrorWithDetails(coreHttp.ErrInternal, "error", err.Error())
	}

	r.logger.Debug("session meta saved", "jwtID", jti.String())
	return nil
}
...
func (r *tokenRepository) GetSessionMeta(ctx context.Context, jti uuid.UUID) (*domain.SessionMeta, error) {
	...
	if errors.Is(err, redis.Nil) {
		r.logger.Warn("session meta not found", "jwtID", jti.String())
		return nil, coreHttp.ErrSessionMetaNotFound
	}
	if err != nil {
		r.logger.Error("error getting session meta", "error", err.Error())
		return nil, coreHttp.NewErrorWithDetails(coreHttp.ErrInternal, "error", err.Error())
	}

	var meta domain.SessionMeta
	if err = json.Unmarshal(data, &meta); err != nil {
		r.logger.Error("error unmarshalling session meta", "error", err.Error())
		return nil, coreHttp.NewErrorWithDetails(coreHttp.ErrInternal, "error", err.Error())
	}

	r.logger.Debug("session meta found", "jwtID", jti.String())
	return &meta, nil
}
...
func (r *tokenRepository) UpdateSessionMeta(ctx context.Context, jti uuid.UUID, ip string) error {
	...
	r.logger.Debug("session meta updated", "jwtID", jti.String())
	return nil
}
...
func (r *tokenRepository) DeleteSessionMeta(ctx context.Context, jti uuid.UUID) error {
	...
	if err != nil {
		r.logger.Error("error deleting session meta", "error", err.Error())
		return coreHttp.NewErrorWithDetails(coreHttp.ErrInternal, "error", err.Error())
	}

	r.logger.Debug("session meta deleted", "jwtID", jti.String())
	return nil
}
